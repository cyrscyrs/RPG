...
public class Sword_Skill_Controller : MonoBehaviour
{
    ...
    private float freezeTimeDuration = 1f;
    ...
    private void Update()
    {
        ...
        if (isReturning)
        {
            ...
            if (Vector2.Distance(transform.position, player.transform.position) < 1)
                player.skill.sword.CatchTheSword();
        }
    ...
    private void BounceLogic()
    {
        if (isBouncing && enemyTarget.Count > 0)
        {
            ...
            {
                //enemyTarget[targetIndex].GetComponent<Enemy>().DamegedEffect();
                SwordSkillDamage(enemyTarget[targetIndex].GetComponent<Enemy>());
                //player.stats.DoPhysicsDamage(enemyTarget[targetIndex].GetComponent<CharacterStats>());
                targetIndex++;
                bounceAmount--;
